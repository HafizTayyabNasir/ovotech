...
# Let's fix the reverse script!
with open('update_all_reverse.py', 'r', encoding='utf-8') as f:
    text = f.read()

# Replace the s4_start and s4_new definitions in the old script
import re as regex_mod
text = regex_mod.sub(r"s4_start = '<div className=\"grid grid-cols-2 md:grid-cols-4 gap-4 mb-8\">'", "s4_start = 'Fictional demonstration</span></div> <div className=\"grid grid-cols-2 md:grid-cols-4 gap-4 mb-8\">'", text)
text = text.replace('s4_new = r\'\'\'<div className="flex flex-col gap-6 mb-8">', 's4_new = r\'\'\'Fictional demonstration</span></div>\n<div className="flex flex-col gap-6 mb-8">')

with open('update_all_reverse2.py', 'w', encoding='utf-8') as f:
    f.write(text)

print("Fixed update_all_reverse2.py")
