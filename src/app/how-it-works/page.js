      ...
      <section style={{ padding: "80px 0", background: "#fff" }}>
        <div className="site-container">
          <div style={{ textAlign: "center", marginBottom: "60px" }}>
            <span style={{ fontSize: "12px", fontWeight: 700, letterSpacing: "2px", color: "#362D7E", textTransform: "uppercase" }}>Step-by-Step Workflow</span>
            <h2 style={{ fontSize: "clamp(28px, 4vw, 38px)", fontWeight: 800, color: "#1A1A1A", marginTop: "8px" }}>The 8-Stage Document Processing Workflow</h2>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "28px", maxWidth: "1000px", margin: "0 auto" }}>
            {detailedSteps.map((s) => (
              <div key={s.step} style={{ background: "#F7F7FA", padding: "32px", borderRadius: "20px", border: "1px solid #E8E8EF" }}>
                <span style={{ fontSize: "28px", fontWeight: 800, color: "#362D7E", display: "block", marginBottom: "12px" }}>{s.step}</span>
                <h3 style={{ fontSize: "18px", fontWeight: 700, color: "#1A1A1A", marginBottom: "8px" }}>{s.title}</h3>
                <p style={{ fontSize: "14px", color: "#555", lineHeight: 1.6 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
