          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {links.map((link, i) => (
                  <Link key={i} href={link.href} style={{ fontSize: "14px", color: "rgba(255,255,255,0.6)", transition: "color 0.2s" }} onMouseEnter={e => e.currentTarget.style.color = "#00A8E8"} onMouseLeave={e => e.currentTarget.style.color = "rgba(255,255,255,0.6)"}>
                    {link.label}
                  </Link>
                ))}
              </div>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
        <div className="site-container" style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", padding: "20px 0", gap: "16px", fontSize: "14px", color: "rgba(255,255,255,0.5)" }}>
          ...
          <div style={{ display: "flex", gap: "24px" }}>
            <Link href="/privacy-policy" style={{ transition: "color 0.2s" }} onMouseEnter={e => e.currentTarget.style.color = "#00A8E8"} onMouseLeave={e => e.currentTarget.style.color = "rgba(255,255,255,0.5)"}>Privacy Policy</Link>
            <Link href="/terms" style={{ transition: "color 0.2s" }} onMouseEnter={e => e.currentTarget.style.color = "#00A8E8"} onMouseLeave={e => e.currentTarget.style.color = "rgba(255,255,255,0.5)"}>Terms &amp; Conditions</Link>
          </div>
        </div>
      </div>
    </footer>
