...
export default function Footer() {
  return (
    <footer style={{ background: "#F8FAFC", color: "#0F172A", paddingTop: "80px", borderTop: "1px solid #E2E8F5" }}>
      <div className="site-container" style={{ display: "flex", flexWrap: "wrap", gap: "60px", paddingBottom: "60px" }}>
        {/* Brand */}
        <div style={{ flex: "0 1 280px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
            <img src="/logo.png" alt="Ovotech Logo" style={{ height: "42px", width: "auto" }} />
          </div>
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            {[
              ...
              <a key={i} href="#" aria-label={s.label} style={{ width: "36px", height: "36px", background: "#FFFFFF", border: "1px solid #E2E8F5", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.3s" }} onMouseEnter={e => { e.currentTarget.style.background = "#02ACEA"; e.currentTarget.style.borderColor = "#02ACEA"; }} onMouseLeave={e => { e.currentTarget.style.background = "#FFFFFF"; e.currentTarget.style.borderColor = "#E2E8F5"; }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="#0F172A"><path d={s.path} /></svg>
              </a>
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: "1 1 500px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "32px" }}>
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 style={{ fontSize: "14px", fontWeight: 800, marginBottom: "20px", color: "#0F172A" }}>{title}</h4>
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {links.map((link, i) => (
                  <Link key={i} href={link.href} style={{ fontSize: "14px", color: "#475569", transition: "color 0.2s", fontWeight: 500 }} onMouseEnter={e => e.currentTarget.style.color = "#02ACEA"} onMouseLeave={e => e.currentTarget.style.color = "#475569"}>
                    {link.label}
                  </Link>
                ))}
              </div>
      ...
      {/* Bottom Bar */}
      <div style={{ borderTop: "1px solid #E2E8F5", background: "#FFFFFF" }}>
        <div className="site-container" style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", padding: "20px 0", gap: "16px", fontSize: "14px", color: "#64748B" }}>
          <span>&copy; 2025 Ovotech. All rights reserved.</span>
          <div style={{ display: "flex", gap: "24px" }}>
            <Link href="/privacy-policy" style={{ transition: "color 0.2s" }} onMouseEnter={e => e.currentTarget.style.color = "#02ACEA"} onMouseLeave={e => e.currentTarget.style.color = "#64748B"}>Privacy Policy</Link>
            <Link href="/terms" style={{ transition: "color 0.2s" }} onMouseEnter={e => e.currentTarget.style.color = "#02ACEA"} onMouseLeave={e => e.currentTarget.style.color = "#64748B"}>Terms &amp; Conditions</Link>
          </div>
        </div>
      </div>
    </footer>
