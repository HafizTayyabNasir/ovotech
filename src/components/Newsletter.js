        ...
        <div className="animate-fadeInUp" style={{ maxWidth: "560px", margin: "0 auto", textAlign: "center" }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexWrap: "wrap", gap: "12px", marginBottom: "16px" }}>
            <input
              ...
              required
            />
            <button type="submit" style={{ flexShrink: 0, background: "#00A8E8", color: "#fff", fontWeight: 600, fontSize: "14px", padding: "14px 28px", borderRadius: "12px", border: "none", cursor: "pointer", transition: "background 0.3s" }} onMouseEnter={e => e.currentTarget.style.background = "#0284C7"} onMouseLeave={e => e.currentTarget.style.background = "#00A8E8"}>
              Subscribe
            </button>
          </form>
          <label style={{ display: "flex", alignItems: "flex-start", gap: "8px", justifyContent: "center", fontSize: "12px", color: "#64748B", cursor: "pointer" }}>
            <input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} style={{ marginTop: "2px", accentColor: "#00A8E8" }} />
