...
// Helper to escape regex special characters (only if using string in RegExp constructor, but we will construct strings manually if needed, or just use string literal replace if unique).
function escapeRegExp(string) {
  return string.replace(/[.*+?^\${}()|[\\]\\\\]/g, '\\\\$&'); // $& means the whole matched string
}

function doReplace(startStr, endStr, newContent, label) {
  // Let's use a regex that matches startStr (ignoring exact whitespace) to endStr
  // Actually, indexOf fails maybe because of quotes or something?
  // Let's just create a regex that ignores all whitespace variations!
  
  const buildPattern = (str) => {
    return str.trim().split(/\\s+/).map(escapeRegExp).join('\\\\s+');
  };

  const pattern = buildPattern(startStr) + "[\\\\s\\\\S]*?" + buildPattern(endStr);
  const regex = new RegExp(pattern);
  
  if (regex.test(content)) {
    content = content.replace(regex, startStr + '\\n' + newContent + '\\n' + endStr);
    console.log("Replaced:", label);
  } else {
    console.log("FAILED to find:", label);
  }
}

// 1. Swap the hero component
content = content.replace("import InteractiveWorkflowShowcaseDark from \\"\\@/components/InteractiveWorkflowShowcaseDark\\";", "import InteractiveWorkflowShowcaseThird from \\"\\@/components/InteractiveWorkflowShowcaseThird\\";");
content = content.replace("<InteractiveWorkflowShowcaseDark />", "<InteractiveWorkflowShowcaseThird />");

// Wait, InteractiveWorkflowShowcaseDark is NOT in the file currently! So let's insert it manually if not found.
if (!content.includes('<InteractiveWorkflowShowcaseThird />')) {
    const s4Marker = '      {/* SECTION 4 — FOR PRACTICE MANAGERS */}';
    let iS4 = content.indexOf(s4Marker);
    if (iS4 !== -1) {
...
if (!content.includes('import InteractiveWorkflowShowcaseThird')) {
...
// 2. Section 2
const s2Start = '<div className="flex overflow-x-auto lg:grid lg:grid-cols-4 gap-6 pb-8 snap-x no-scrollbar">';
const s2End = '<div className="text-center text-[12px] text-[#64748B]">';
const s2New = `          <div className="w-full py-12 relative overflow-x-auto no-scrollbar">
...
`;
// Actually, since I'm appending startStr and endStr around newContent, I should just make s2Start and s2End NOT part of what is replaced, OR replace them fully. My doReplace replaces startStr to endStr INCLUSIVE. So s2New MUST NOT include startStr, but I am concatenating startStr + newContent + endStr in doReplace.
// Wait! The user's requested layout for Section 2 *replaces* the grid container entirely.
// Let's modify doReplace to NOT append startStr/endStr, but we just pass the exact markers we want to be removed and replace the WHOLE block with s2New!
function doReplaceBlock(startStr, endStr, replacementStr, label) {
  const buildPattern = (str) => {
    return str.trim().split(/\\s+/).map(escapeRegExp).join('\\\\s+');
  };
  const pattern = buildPattern(startStr) + "[\\\\s\\\\S]*?" + buildPattern(endStr);
...
doReplaceBlock(s2Start, s2End, s2Repl, "Section 2");


// 3. Section 3
const s3Start = '<div className="bg-[#FFFFFF] rounded-3xl border border-[#E0E8F5] shadow-lg overflow-hidden">';
const s3End = '      {/* SECTION 4 — FOR PRACTICE MANAGERS */}';
const s3NewBlock = `<div className="flex flex-col gap-12">
            {[
              {
...
doReplaceBlock(s3Start, s3End, s3NewBlock, "Section 3");

// 4. Section 4
const s4Start = '<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">';
const s4End = '<div className="flex flex-wrap gap-3 justify-center text-[12px] font-bold text-[#0A1838] mb-6">';
const s4New = `<div className="flex flex-col gap-6 mb-8">
              {/* Radial & Bar Gauges */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
...
doReplaceBlock(s4Start, s4End, s4New, "Section 4");

// 5. Section 5
const s5Start = '<div className="flex gap-3 mb-6 overflow-x-auto no-scrollbar pb-2">';
const s5End = '<div className="mt-4 text-center text-[11px] text-[#94A3B8]">';
const s5New = `<div className="flex gap-3 mb-6 overflow-x-auto no-scrollbar pb-2">
...
doReplaceBlock(s5Start, s5End, s5New, "Section 5");


// 6. Section 7
const s7Start = '<div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">';
const s7End = '<div className="text-center">';
const s7New = `<div className="relative mb-12 bg-white border border-[#E0E8F5] rounded-3xl overflow-hidden shadow-lg">
...
doReplaceBlock(s8Start, s8End, s8NewBlock, "Section 8");
...
doReplaceBlock(s10Start, s10End, s10New, "Section 10");


// 9. Section 11
const s11Start = '<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">';
const s11End = '      {/* SECTION 12';
const s11NewBlock = `<div className="flex flex-col gap-12">
      ...
      {/* SECTION 12`;
doReplaceBlock(s11Start, s11End, s11NewBlock, "Section 11");
...
doReplaceBlock(s12Start, s12End, s12New, "Section 12");


// 11. Section 13 Layers
...
doReplaceBlock(s13Start, s13End, s13New, "Section 13 Layers");

// 12. Section 13 Badges
const s13bStart = '<div className="flex flex-wrap justify-between items-center max-w-4xl mx-auto gap-6 lg:gap-0">';
const s13bEnd = '      {/* SECTION 14';
const s13bNewBlock = `<div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
      ...
      {/* SECTION 14`;
doReplaceBlock(s13bStart, s13bEnd, s13bNewBlock, "Section 13 Badges");
