...
replacements.append((s2_start, s2_end, s2_new))
...
replacements.append((s3_start, s3_end, s3_new))
...
s4_new = r'''Fictional demonstration</span></div>
...
replacements.append((s4_start, s4_end, s4_new))
...
replacements.append((s5_start, s5_end, s5_new))
...
s7_new = r'''<div className="relative mb-12 bg-white border border-[#E0E8F5] rounded-3xl overflow-hidden shadow-lg">
          ...
          <div className="text-center">'''
replacements.append((s7_start, s7_end, s7_new))
...
replacements.append((s8_start, s8_end, s8_new))
...
replacements.append((s10_start, s10_end, s10_new))
...
s11_new = r'''<div className="flex flex-col gap-12 w-full">
      ...
      {/* SECTION 12'''
replacements.append((s11_start, s11_end, s11_new))
...
replacements.append((s12_start, s12_end, s12_new))
...
replacements.append((s13_start, s13_end, s13_new))
...
s13b_new = r'''<div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto w-full">
      ...
      {/* SECTION 14'''
replacements.append((s13b_start, s13b_end, s13b_new))

for s_start, s_end, s_new in reversed(replacements):
    pattern = build_pattern(s_start) + r'[\s\S]*?' + build_pattern(s_end)
    if not re.search(pattern, content):
        print("FAILED to find:", s_start[:30])
        continue
    content = re.sub(pattern, s_new, content, count=1)
    print("Replaced:", s_start[:30])

with open(path, 'w', encoding='utf-8') as f:
    f.write(content)

print("All Python replacements applied in reverse.")
